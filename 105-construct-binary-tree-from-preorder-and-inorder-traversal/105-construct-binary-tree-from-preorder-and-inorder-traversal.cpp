...
public:
    TreeNode* constructTree(vector<int>& preorder, int pStart, int pEnd, vector<int>& inorder, int iStart, int iEnd, map<int, int>&mp){
        ...
        root->left=constructTree(preorder, pStart+1, pStart+nElem, inorder, iStart, elem-1, mp);
        root->right=constructTree(preorder, pStart+nElem+1, pEnd, inorder, elem+1, iEnd, mp);
        return root;    
    }
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        int pStart=0, pEnd=preorder.size()-1, iStart=0, iEnd=inorder.size()-1;
        map<int, int>mp;
